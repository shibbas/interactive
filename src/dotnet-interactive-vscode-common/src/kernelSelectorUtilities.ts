...
export function getKernelSelectorOptions(kernel: CompositeKernel, document: vscodeLike.NotebookDocument): KernelSelectorOption[] {
    ...
    for (const item of notebookMetadata.kernelInfo.items) {
        const kernelInfo: contracts.KernelInfo = {
            ...
            // a few lines down we filter kernels to only those that support `SubmitCode` so we artificially add that
            // value here to ensure we show kernels that haven't been re-connected yet
            supportedKernelCommands: [{ name: contracts.SubmitCodeType }],
            supportedDirectives: []
        };
    ...
    // ...filter to only kernels that can handle `SubmitCode`...
    const filteredKernels = [...kernelInfos.values()].filter(k => k.supportedKernelCommands.findIndex(kci => kci.name === contracts.SubmitCodeType) >= 0);
    ...
    const selectorOptions: KernelSelectorOption[] = filteredKernels.map(kernelInfo => {
        const result: KernelSelectorOption = {
            ...
            displayValue: kernelInfo.displayName
        };
